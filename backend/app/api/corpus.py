...
from app.ingestion.jobs import _source_file_absolute_path, delete_uploaded_source
...
class CorpusItem(BaseModel):
    ...
    latest_ingestion_job: dict[str, Any] | None = None
...
class PriorityRequestItem(BaseModel):
    ...
    reviewed_by_external_user_id: str | None = None
...
class IngestionJobItem(BaseModel):
    ...
    eta_window: EtaWindowItem | None = None
    wait_window: EtaWindowItem | None = None
    eta_confidence: str | None = None
    queue_position: int | None = None
    jobs_ahead: int | None = None
    queue_delay_message: str | None = None
    source_file_name: str | None = None
    source_type: str | None = None
    file_size_bytes: int | None = None
    corpus_name: str | None = None
    priority_request: PriorityRequestItem | None = None
...
class ChunkContextResponse(BaseModel):
    ...
    chunk_id: int
    freshness: dict[str, Any]
    target: ChunkContextItem | None = None
...
class DbConnectorItem(BaseModel):
    ...
    status: str
    last_cursor_updated_at: str | None = None
...
def _connector_payload(row: DbConnectorRow) -> DbConnectorItem:
    return DbConnectorItem(
        ...
        else ("syncing" if row.status == "syncing" else "healthy"),
    )
...
def connector_request_list_endpoint(
    ...
    return [
        _connector_request_payload(row)
        for row in list_connector_requests(requester_external_user_id=requester_id)
    ]
...
def connector_request_review_endpoint(
    request_id: int, body: ConnectorRequestReview, _admin=Depends(require_admin_user)
):
    actor = get_current_user()
    ...
    if request is None:
        raise HTTPException(
            status_code=404,
            detail={"error": "connector_request_not_found", "request_id": request_id},
        )
    insert_admin_audit_event(
        event_type="connector",
...
def db_connector_create_endpoint(body: DbConnectorCreate, _admin=Depends(require_admin_user)):
    ...
    connector_id = upsert_db_connector(
        ...
        corpus_name=body.corpus_name.strip()
        if body.corpus_name and body.corpus_name.strip()
        else None,
...
def db_connector_schedule_endpoint(
    connector_id: int, body: DbConnectorScheduleUpdate, _admin=Depends(require_admin_user)
):
    ...
    if connector is None:
        raise HTTPException(
            status_code=404, detail={"error": "connector_not_found", "connector_id": connector_id}
        )
...
def db_connector_sync_endpoint(
    connector_id: int, body: DbConnectorSyncRequest, _admin=Depends(require_admin_user)
):
    try:
        result = ingest_db_connector(connector_id, row_limit=body.row_limit)
    except ValueError as exc:
        raise HTTPException(
            status_code=404, detail={"error": "connector_not_found", "message": str(exc)}
        ) from exc
    except ConnectorSyncConflict as exc:
        raise HTTPException(
            status_code=409, detail={"error": "connector_sync_in_progress", "message": str(exc)}
        ) from exc
    except Exception as exc:
        raise HTTPException(
            status_code=400, detail={"error": "connector_sync_failed", "message": str(exc)}
        ) from exc
    insert_admin_audit_event(
        event_type="connector",
...
def db_connector_schema_endpoint(connector_id: int, _admin=Depends(require_admin_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail={"error": "connector_schema_failed", "message": str(exc)}
        ) from exc


@router.post("/connectors/db/{connector_id}/preview")
def db_connector_preview_endpoint(
    connector_id: int, body: DbConnectorSyncRequest, _admin=Depends(require_admin_user)
):
    connector = next((row for row in list_db_connectors() if row.id == connector_id), None)
    if connector is None:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail={"error": "connector_preview_failed", "message": str(exc)}
        ) from exc
...
def submit_priority_request(job_id: int, body: QueuePriorityRequestCreate):
    ...
    if actor is None:
        raise HTTPException(
            ...
            detail={
                ...
                "message": "Authentication is required before submitting a priority request.",
            },
        )
    if (
        job.owner_external_user_id
        and actor.user_id != job.owner_external_user_id
        ...
        raise HTTPException(
            ...
            detail={
                "error": "not_job_owner",
                "message": "Only the original uploader can request priority for this job.",
            },
        )
    if str(job.status).lower() not in {"queued", "processing", "running", "paused"}:
        raise HTTPException(
            status_code=400,
            detail={
                "error": "job_not_actionable",
                "message": "Priority can be requested only while indexing is still active or waiting.",
            },
        )
    if not (job.owner_external_user_id or job.owner_email):
        raise HTTPException(
            ...
            detail={
                "error": "owner_missing",
                "message": "This job does not have a recorded owner for user-side priority requests.",
            },
        )
...
def delete_corpus_source_endpoint(source_id: int):
    try:
        ...
        raise HTTPException(
            status_code=404, detail={"error": "source_not_found", "source_id": source_id}
        ) from None
...
def corpus_source_file_endpoint(source_id: int):
    ...
    if not absolute_path.exists():
        ...
        if metadata.get("seed_pack") == "public_demo":
            ...
            body = "\n\n".join(
                str(chunk.get("chunk_text") or "").strip() for chunk in chunks
            ).strip()
            if body:
    ...
    media_type = source.mime_type or (
        "application/pdf" if source.source_type == "pdf" else "application/octet-stream"
    )
