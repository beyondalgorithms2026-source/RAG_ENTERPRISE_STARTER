...
def auth_login(next_path: str = Query(default_factory=lambda: settings.FRONTEND_APP_URL)):
    if not auth_required():
        raise HTTPException(
            status_code=503,
            detail={"error": "auth_disabled", "message": "Authentication is disabled."},
        )
...
def auth_callback(request: Request, code: str, state: str):
    ...
    if expected_state != state:
        raise HTTPException(
            status_code=400,
            detail={"error": "invalid_state", "message": "OIDC state cookie mismatch."},
        )
    try:
        ...
        if not access_token:
            raise AuthError(
                "oidc_token_missing", "OIDC token response did not include an access token", 502
            )
    ...
    response = RedirectResponse(
        url=state_payload.get("next_path") or settings.FRONTEND_APP_URL, status_code=302
    )
...
def auth_local_dev_login(payload: LocalDevLoginRequest):
    if not local_dev_auth_enabled():
        raise HTTPException(
            status_code=404,
            detail={"error": "not_found", "message": "Local dev login is not enabled."},
        )
    user = authenticate_local_dev_user(payload.email, payload.password)
    if user is None:
        raise HTTPException(
            status_code=401,
            detail={"error": "invalid_credentials", "message": "Invalid local dev credentials."},
        )
...
def auth_local_dev_assume(payload: LocalDevAssumeRequest):
    if not local_dev_auth_enabled():
        raise HTTPException(
            status_code=404,
            detail={"error": "not_found", "message": "Local dev assume is not enabled."},
        )
    email = payload.email.strip().lower()
    if not email:
        raise HTTPException(
            status_code=400, detail={"error": "invalid_email", "message": "Email is required."}
        )
