    ...
    def status(self) -> GraphStoreStatus:
        if settings.ENABLE_GRAPH and settings.BUILD_GRAPH_ON_INGEST:
            return GraphStoreStatus(
                backend="noop", enabled=False, reason="graph_store_unavailable"
            )
...
class SourceMetadataGraphStore:
    def status(self) -> GraphStoreStatus:
        if settings.ENABLE_GRAPH and settings.BUILD_GRAPH_ON_INGEST:
            return GraphStoreStatus(
                backend="source_metadata_json", enabled=True, reason="m14_metadata_store_ready"
            )
        return GraphStoreStatus(
            backend="source_metadata_json", enabled=False, reason="graph_build_disabled"
        )

    def compact_artifact(self, *, graph_status: Any) -> dict[str, Any]:
        return {
            "artifact_version": getattr(
                graph_status, "artifact_version", GRAPH_STORE_ARTIFACT_VERSION
            ),
