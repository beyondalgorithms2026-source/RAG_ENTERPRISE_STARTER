...
def build_resolved_profile_bundle(
...
def _validated_selected_profiles(selected_profiles: dict[str, str]) -> dict[str, str]:
    ...
    missing = [
        profile_type
        for profile_type in PROFILE_TYPES_FOR_TUNING
        if not normalized.get(profile_type)
    ]
    if missing:
    ...
    for profile_type in ("embedding", "reranker", "llm"):
        ...
        if not is_registry_approved_profile(profile_type, profile_name):
            raise ValueError(
                f"Profile '{profile_name}' of type '{profile_type}' is not an approved registry option"
            )
    ...
    *,
    ...
    with engine.begin() as conn:
        next_id = conn.execute(
            text("SELECT COALESCE(MAX(id), 0) + 1 FROM tuning_config_versions")
        ).scalar_one()
...
def get_candidate_draft(draft_id: int) -> dict[str, Any] | None:
    sql = text(
        """
        SELECT id, version_label, config_kind, status, name, description,
               selected_profiles_json, resolved_config_json, lineage_json,
...
def promote_candidate_to_live(
    *,
    ...
    if retrieval_override_config:
        ...
        resolved_retrieval = ((draft.get("resolved_config") or {}).get("retrieval") or {}).get(
            "config"
        ) or {}
        ...
        upsert_profile(
            "retrieval", promoted_retrieval_profile_name, config_to_promote, is_default=False
        )
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            text(
                "UPDATE tuning_config_versions SET status = 'archived' WHERE config_kind = 'live' AND version_label <> :version_label"
            ),
            {"version_label": version_label},
        )
        conn.execute(
    ...
    payload["lineage"] = payload.pop("lineage_json", {})
    return {
        "promoted_version": payload,
        "live_configuration": live_after,
        "previous_live_configuration": live_before,
    }
...
def create_embedding_experiment(
    *,
    ...
    if not warning_acknowledged or confirmation_count < 2:
        raise ValueError(
            "Embedding experiments require warning acknowledgement and double confirmation"
        )
