...
class FeedbackFlywheelAR12Tests(SmokeTestBase):
    ...
    def _seed_chunk_and_thumbsdown(self):
            ...
            [
                {
                    ...
                    "heading": "Escalation",
                    "section_path": "p:0",
                    "chunk_text": f"{token} escalation policy: contact the duty manager within one hour.",
                    "token_count": 10,
                    "locator_json": {},
                    "provenance_json": {},
                }
        ...
        with engine.connect() as conn:
            chunk_id = conn.execute(
                text("SELECT id FROM chunks WHERE source_id = :s ORDER BY chunk_index"),
                {"s": source_id},
            ).scalar_one()
        ...
        insert_trace(
            ...
            latency_ms={},
            score_diagnostics=[],
            trace_json={"cited_chunk_ids": [chunk_id], "acl": {"accessed_doc_ids": [source_id]}},
            active_profiles={},
        )
    ...
    def _delete_event(self, event_id, request_id):
        with engine.begin() as conn:
            ...
            conn.execute(
                text("DELETE FROM retrieval_traces WHERE request_id = :r"), {"r": request_id}
            )
            conn.execute(
                text(
                    "DELETE FROM query_failure_clusters WHERE label = (SELECT label FROM query_failure_clusters WHERE id = id LIMIT 0)"
                )
            )
    ...
    def _cluster_for(self, question):
        ...
        match = next(
            (
                c
                for c in clusters
                if normalize_question(c["label"]) == target
                or target
                in json.dumps(c.get("sample_questions_json") or c.get("sample_questions") or [])
            ),
            None,
        )
    ...
    def _delete_cluster(self, cluster_id):
        with engine.begin() as conn:
            conn.execute(
                text("DELETE FROM query_failure_clusters WHERE id = :id"), {"id": cluster_id}
            )
    ...
    def test_full_path_thumbsdown_to_gating_eval_case(self):
        ...
        self.assertEqual(
            summary["by_review_status"].get("unreviewed"), len(proposed["proposed_cases"])
        )
        ...
        try:
            ...
            before = run_pack_eval(
                pack_paths=[PACKS_DIR / f"pack_{pack_name}.json"],
                modes=("hybrid",),
                gate_mode="hybrid",
            )
            ...
            review_pack_case(
                pack_name, case["id"], relevant={str(chunk_id): 3}, reviewer="reviewer@example.com"
            )
            ...
            gated_ids = [
                c["case_id"]
                for c in after["packs"][0]["cases"]
                if c["mode"] == "hybrid" and c["review_status"] != "unreviewed"
            ]
    ...
    def test_reviewed_case_requires_relevance_map(self):
        question, _chunk_id = self._seed_chunk_and_thumbsdown()
        cluster_id = self._cluster_for(question)
        pack_name = self._temp_pack()
