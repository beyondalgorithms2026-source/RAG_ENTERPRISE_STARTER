...
class SourceDownloadAR20Tests(SmokeTestBase):
    ...
    def _admin_client(self):
        ...
        main_module.authenticate_request = lambda request: AuthenticatedUser(
            user_id="ar20", email="ar20@example.com", roles=["admin"], groups=["ops"]
        )
        self.addCleanup(
            lambda: (
    ...
    def _seed_source(
        self, *, storage_path: str, file_name: str, size: int, source_type: str = "txt"
    ):
    ...
    def test_download_info_flags_large_file(self):
        ...
        small = self._seed_source(
            storage_path=f"tests/ar20-small-{uuid4().hex[:8]}.txt",
            file_name="small.txt",
            size=1024,
        )
        big = self._seed_source(
            storage_path=f"tests/ar20-big-{uuid4().hex[:8]}.pdf",
            file_name="big.pdf",
            size=200 * 1024 * 1024,
            source_type="pdf",
        )
        info_small = client.get(
            f"/admin/sources/{small}/download-info", headers={"Authorization": "Bearer t"}
        ).json()
        info_big = client.get(
            f"/admin/sources/{big}/download-info", headers={"Authorization": "Bearer t"}
        ).json()
    ...
    def test_download_streams_existing_repo_file(self):
        ...
        sid = self._seed_source(
            storage_path="README.md", file_name="README.md", size=1024, source_type="txt"
        )
    ...
    def test_download_missing_file_is_404(self):
        client = self._admin_client()
        sid = self._seed_source(
            storage_path="does/not/exist.pdf", file_name="x.pdf", size=10, source_type="pdf"
        )
        resp = client.get(f"/admin/sources/{sid}/download", headers={"Authorization": "Bearer t"})
        ...
        with engine.begin() as conn:
            conn.execute(
                text(
                    "UPDATE sources SET source_metadata_json = CAST(:meta AS jsonb) WHERE id = :id"
                ),
    ...
    def test_download_unknown_source_is_404(self):
        client = self._admin_client()
        resp = client.get(
            "/admin/sources/99999999/download", headers={"Authorization": "Bearer t"}
        )
        self.assertEqual(resp.status_code, 404)
