...
class SemanticCacheSimilarityAR6Tests(unittest.TestCase):
    ...
    def setUp(self):
        self.actor = AuthenticatedUser(
            user_id=f"ar6-{uuid4().hex[:8]}",
            email="ar6@example.test",
            roles=["admin"],
            groups=["employees"],
        )
        ...
        with engine.begin() as conn:
            version_ids = [
                ...
                for r in conn.execute(
                    text(
                        "SELECT id FROM semantic_cache_policy_versions WHERE policy_id = ANY(:p)"
                    ),
                    {"p": self.policy_ids},
                ).fetchall()
            ]
            if version_ids:
                conn.execute(
                    text(
                        "DELETE FROM semantic_cache_policy_events WHERE policy_version_id = ANY(:v)"
                    ),
                    {"v": version_ids},
                )
                conn.execute(
                    text("DELETE FROM semantic_cache_entries WHERE policy_version_id = ANY(:v)"),
                    {"v": version_ids},
                )
            conn.execute(
                text("DELETE FROM semantic_cache_policies WHERE id = ANY(:p)"),
                {"p": self.policy_ids},
            )
    ...
    def _store(self, draft, namespace):
        return store_cache_entry(
            ...
            answer_json={
                "answer": "Twenty days of annual leave.",
                "used_chunks_count": 1,
                "mode": "hybrid",
            },
            citations_json=[],
    ...
    def test_dead_threshold_field_removed_from_retrieval_profile(self):
        self.assertNotIn(
            "semantic_cache_similarity_threshold", RetrievalProfileConfig().model_dump()
        )

    def test_policy_config_accepts_and_validates_match_mode_and_threshold(self):
        cfg = validate_policy_config(
            {"allow_questions": ["x"], "match_mode": "semantic", "similarity_threshold": 0.88}
        )
        ...
        with self.assertRaisesRegex(ValueError, "similarity_threshold"):
            validate_policy_config(
                {"allow_questions": ["x"], "match_mode": "semantic", "similarity_threshold": 0.2}
            )
    ...
    def test_semantic_policy_stores_query_embedding(self):
        ...
        with engine.connect() as conn:
            emb = conn.execute(
                text("SELECT query_embedding_json FROM semantic_cache_entries WHERE id = :id"),
                {"id": stored["id"]},
            ).scalar_one()
    ...
    def test_paraphrase_hits_under_semantic_mode(self):
        ...
        para = get_cache_entry(
            question="How much leave time do I get?",
            retrieval_mode="hybrid",
            actor=self.actor,
            policy=draft,
            cache_namespace=namespace,
