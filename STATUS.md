...
**Current UX milestone:** UX10 complete (2026-06-17) — finish pass. **UX-series (UX0–UX10) is complete.** Replaced prose placeholders ("Unavailable"/"Captured") with em dashes; relabeled the internal "Path:" metric to "Route:" with a plain-language tooltip; added a standard product glossary to `web/DESIGN.md §5b`; moved the long inline access-request form into a focusable modal/dialog (trigger button + `role="dialog"`, Escape/backdrop/close, opened per message); added an explicit CSS spinner (`.app-icon.spin`) for the search loading state; added an evidence-card hover state; and wired **⌘/Ctrl+Enter** to submit the chat composer (with an accessible textarea label). `next build` 12/12 + `tsc --noEmit` green; no-external-deps invariant holds.

**Prior UX milestone:** UX9 complete (2026-06-17) — responsiveness pass. Added consolidated intermediate breakpoints (1280 / 1024 / 768) at the end of the `globals.css` responsive section (ordered broad→narrow so the narrowest wins): at ≤1280 the chat layout/metadata tighten; at ≤1024 the evidence rail drops below the conversation as a full-width sheet, the chat metadata bar wraps, admin 4/5-col filter grids → 2-col, the search query input goes full-width; at ≤768 filter grids → 1-col, chat answer card / search page paddings shrink, search facets stack, snippet max-width removed. `next build` 12/12 + `tsc --noEmit` green; no-external-deps invariant holds. Next: UX10 (finish pass).

**Prior UX milestone:** UX8 complete (2026-06-17) — accessibility baseline. Global `:focus-visible` ring on every interactive element (focus-visible rules 5→11); `aria-live="polite"` on the chat answer article + `role="status"` on the progress card (with the % marked `aria-hidden`); `role="alert"` on chat/search error banners and `role="status"` on the search loading state; accessible name added to the only unnamed icon-only button (chat "New thread"); fake search inputs taken out of the tab order (`tabIndex={-1}`). Contrast audited programmatically against the UX0 palette — **all key pairs pass WCAG AA** (lowest muted-on-surface-high 4.96), so no token change needed. `next build` 12/12 + `tsc --noEmit` green; no-external-deps invariant holds. Next: UX9 (responsiveness pass).

**Prior UX milestone:** UX7 complete (2026-06-17) — IA & control hygiene. Admin sidebar grouped into collapsible sections (Operate / Retrieval / Data / Governance) with Overview pinned (`lib/admin-nav.ts` `groupAdminNav`; pure module so the client shell can import it). Workspace nav vocabulary reconciled to one verb set — sidebar now Ask / Search / History (Search surfaced in the sidebar; "Chat"→"Ask", "Search History"→"History"); guide copy aligned. Fake/disabled controls are **marked "coming soon" (preserved, not removed)** with a consistent treatment (`title="Coming in a later release."`, `(coming soon)` accessible name, `.coming-soon-badge` "Soon" pill on labelled controls / `.is-coming-soon` accent dot on icon-only): ⌘K search, workspace search, Settings ×2, composer attach/image/mic, Export Findings. Pattern documented in `web/DESIGN.md`. `next build` 12/12 + `tsc --noEmit` green; no-external-deps invariant holds. Next: UX8 (accessibility baseline).
